...
# =============================================================================
# ZIVPN (UDP)  — ZIVPN is itself a Hysteria fork, so it reuses the SAME trusted
# Hysteria binary (never the random third-party binary from install guides).
# It runs as a SEPARATE service on ZIVPN's standard base port 5667 with its own
# NON-OVERLAPPING UDP hopping range 6000-19999, so it can't collide with the
# Hysteria option (20000-50000) or SlowDNS (UDP 53). UDP-only + a dedicated
# INPUT/nat rule set => it never touches the TCP protocols (SSH/SSL/WS/V2Ray).
# =============================================================================
ZI_BIN=/usr/local/bin/hysteria      # same fork/binary as the Hysteria option
ZI_DIR=/etc/zivpn
ZI_CONF=$ZI_DIR/config.json
ZI_USERS=$ZI_DIR/users
ZI_OBFS_FILE=$ZI_DIR/obfs
ZI_UNIT=/etc/systemd/system/zivpn.service
ZI_PORT=5667            # ZIVPN standard base UDP port (kept in sync with porthop)
ZI_HOP_LO=6000          # hopping range low  (below Hysteria's 20000, above 53)
ZI_HOP_HI=19999         # hopping range high

zi_active() { systemctl is-active --quiet zivpn 2>/dev/null; }

zi_ensure_cert() {
    [ -s "$ZI_DIR/zivpn.crt" ] && [ -s "$ZI_DIR/zivpn.key" ] && return 0
    mkdir -p "$ZI_DIR"
    openssl req -x509 -nodes -newkey rsa:2048 -days 3650 \
        -subj "/CN=${HOST_DISPLAY:-zivpn}" \
        -keyout "$ZI_DIR/zivpn.key" -out "$ZI_DIR/zivpn.crt" >/dev/null 2>&1
}

# Same rule as the Hysteria option: ZIVPN apps authenticate with the PASSWORD
...
{
  "listen": ":$ZI_PORT",
  "cert": "$ZI_DIR/zivpn.crt",
  "key": "$ZI_DIR/zivpn.key",
  "up_mbps": 100,
  "down_mbps": 100,
  "disable_udp": false,
  "obfs": "$obfs",
...
zi_write_porthop() {
    ...
    cat > /usr/local/bin/zivpn-porthop <<'PHEOF'
#!/bin/bash
# Values MUST match ZI_PORT / ZI_HOP_LO / ZI_HOP_HI in the installer.
export PATH="/usr/sbin:/sbin:/usr/local/sbin:$PATH"
IFACE=$(ip route 2>/dev/null | awk '/^default/{print $5; exit}'); [ -z "$IFACE" ] && IFACE=eth0
LO=6000; HI=19999; PORT=5667
add(){
  iptables -t nat -C PREROUTING -i "$IFACE" -p udp --dport $LO:$HI -j REDIRECT --to-ports $PORT 2>/dev/null || \
    iptables -t nat -A PREROUTING -i "$IFACE" -p udp --dport $LO:$HI -j REDIRECT --to-ports $PORT
  iptables -C INPUT -p udp --dport $PORT -j ACCEPT 2>/dev/null || iptables -A INPUT -p udp --dport $PORT -j ACCEPT
...
case "$1" in up) add;; down) del;; *) echo "usage: $0 up|down";; esac
PHEOF
    chmod +x /usr/local/bin/zivpn-porthop
    if command -v ufw >/dev/null 2>&1 && ufw status 2>/dev/null | grep -qi active; then
        ufw allow "${ZI_PORT}"/udp >/dev/null 2>&1
        ufw allow "${ZI_HOP_LO}:${ZI_HOP_HI}"/udp >/dev/null 2>&1
    fi
}
...
zi_write_unit() {
    cat > "$ZI_UNIT" <<UNIT
[Unit]
Description=ZIVPN UDP Tunnel
After=network.target
[Service]
Type=simple
ExecStartPre=/usr/local/bin/zivpn-porthop up
ExecStart=$ZI_BIN server --config $ZI_CONF
ExecStopPost=/usr/local/bin/zivpn-porthop down
Restart=always
RestartSec=3
LimitNOFILE=1048576
[Install]
...
zi_activate() {
    section "ACTIVATE ZIVPN (UDP)" "$G"
    # ZIVPN uses the same fork as the Hysteria option — reuse its trusted binary.
    if ! hy_install; then err "ZIVPN binary install failed — check the server's internet."; pause; return; fi
    local u p
    read -rp "$(echo -e "  ${C}Username${NC} : ")" u
    [ -z "$u" ] && { err "Username can't be empty."; pause; return; }
    read -rp "$(echo -e "  ${C}Password${NC} : ")" p
    ...
    sleep 2
    if zi_active; then
        ok "ZIVPN user '${W}$u${NC}' added — service running."
        zi_show
    else
        err "ZIVPN failed to start. Last errors:"
        journalctl -u zivpn -n 8 --no-pager 2>/dev/null | sed 's/^/    /'
        grep -v "^${u}:" "$ZI_USERS" > "$ZI_USERS.tmp" 2>/dev/null && mv "$ZI_USERS.tmp" "$ZI_USERS"
        zi_write_config
    fi
    pause
}
...
zi_show() {
    if [ ! -s "$ZI_USERS" ]; then err "No ZIVPN users yet — activate first."; return; fi
    local obfs; obfs=$(cat "$ZI_OBFS_FILE" 2>/dev/null)
    local col="$PINK"; echo ""; line_top "$col"
    crow "$col" "${W}${BOLD}ZIVPN (UDP) CONNECTION${NC}"; line_mid "$col"
    row "$col" "${GR}Server${NC}    ${W}${SERVER_IP}${NC}"
    row "$col" "${GR}UDP ports${NC} ${W}${ZI_HOP_LO}-${ZI_HOP_HI}${NC}"
    row "$col" "${GR}Obfs${NC}      ${W}${obfs}${NC}"
    line_mid "$col"
    crow "$col" "${GR}Users  (username : password)${NC}"
    local uu pp
...
zi_remove_user() {
    if [ ! -s "$ZI_USERS" ]; then err "No users."; pause; return; fi
    echo -e "  ${C}Users:${NC}"; sed 's/:.*//' "$ZI_USERS" | nl -w2 -s') '
    local ru
    read -rp "$(echo -e "  ${P}❯${NC} username to remove : ")" ru
    [ -z "$ru" ] && { note "Cancelled."; pause; return; }
    if grep -q "^${ru}:" "$ZI_USERS"; then
        grep -v "^${ru}:" "$ZI_USERS" > "$ZI_USERS.tmp" && mv "$ZI_USERS.tmp" "$ZI_USERS"
        zi_write_config
        systemctl restart zivpn >/dev/null 2>&1
        ok "Removed '$ru'."
    else
...
zi_deactivate() {
    section "DEACTIVATE ZIVPN" "$R"
    systemctl stop zivpn >/dev/null 2>&1
    systemctl disable zivpn >/dev/null 2>&1
    /usr/local/bin/zivpn-porthop down 2>/dev/null
    ok "ZIVPN deactivated. Users stay saved for next time."
    pause
}

zivpn_menu() {
    while true; do
        section "ZIVPN — UDP" "$PINK"
        local col="$PINK"; line_top "$col"
        if zi_active; then
            row "$col" "${GR}STATUS${NC}  ${G}● active${NC}"
            row "$col" "${GR}PORTS${NC}   ${W}UDP ${ZI_HOP_LO}-${ZI_HOP_HI}${NC}"
            row "$col" "${GR}USERS${NC}   ${W}$(grep -c . "$ZI_USERS" 2>/dev/null || echo 0)${NC}"
        else
            row "$col" "${GR}STATUS${NC}  ${R}○ not active${NC}"
        fi
        line_bot "$col"; echo ""
        menu_item "1" "⚡" "Activate / add user"      "$G"
        menu_item "2" "📋" "Show connection details"  "$PINK"
        menu_item "3" "🗑 " "Remove a user"            "$ORANGE"
        menu_item "4" "🧹" "Deactivate ZIVPN"         "$R"
        menu_item "0" "↩ " "Back to main menu"        "$GR"
        echo ""
        read -rp "$(echo -e "  ${P}❯${NC} select an option : ")" ZO
        case "$ZO" in
            1) zi_activate ;;
            2) zi_show; pause ;;
            3) zi_remove_user ;;
